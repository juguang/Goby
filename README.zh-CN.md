...
| | |
| --- | --- |
| **针对实时网页的工具集** | 25 个内置工具 + MCP Server 扩展 —— 查询、列出、填写、点击（选择器/索引双模式）、勾选、选择、提交、等待、执行 JS、截图、分析、导航、打开新标签、列出标签、关闭标签、结束工作流、检索收藏夹等，还可接入 Tavily/GitHub/Cloudflare 等外部 MCP server。页面就是 agent 的工作台。 |
