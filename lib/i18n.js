...
(function () {
  ...
  function getSystemPrompt() {
    ...
    prompts.zh = [
      ...
      '4. 完整性·何时停 — 区分两类：①查询类（阅读/分析/总结）获取足够信息即可停，用 page_analyze 取摘要、page_query body 或 page_evaluate 取正文；②执行类（翻译全文、填表、批量、列全部）必须按用户指定范围完整覆盖，每段每项都做到，不能因"做了大部分"提前收尾。',
    ...
    prompts.en = [
      ...
      '4. Completeness · when to stop — Distinguish two types: ① Query tasks (read/analyze/summarize) — stop once you have enough info; use page_analyze for a summary, page_query body or page_evaluate for the body text. ② Execution tasks (translate the whole page, fill a form, batch operations, list everything) — you MUST fully cover the scope the user specified, every paragraph and every item; never wrap up early just because "most of it is done".',
