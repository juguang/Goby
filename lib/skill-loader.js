...
(function () {
  ...
  function validateSkill(parseResult) {
    ...
    for (var i = 0; i < parseResult.actions.length; i++) {
      ...
      // 语法检查 — 不用 new Function()（受页面 CSP 限制）
      var code = action.rawCode.trim();
      if (code.length === 0) {
        errors.push('Action "' + action.name + '" 代码为空');
        continue;
      }
      // 自动补全 function 包装（兼容只写函数体的简写形式）
      ...
      if (openBraces !== closeBraces) {
        errors.push('Action "' + action.name + '" 代码花括号不匹配 (开' + openBraces + ' 闭' + closeBraces + ')');
        continue;
      }
      ...
      validatedActions.push({
        ...
        execute: (function () {
          try { return new Function('return (' + action.rawCode + ')')(); }
          catch (e) { return null; }
        })()
      });
    }
