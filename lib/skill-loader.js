...
(function () {
  ...
  function validateSkill(parseResult) {
    ...
    for (var i = 0; i < parseResult.actions.length; i++) {
      ...
      // 语法检查 — 注：不用 new Function()，避免受页面 CSP 限制
      //   (部分网站禁止 'unsafe-eval'，new Function 会抛 EvalError)
      //   这里只做字符串层面检查：验证 function 定义完整性
      ...
      if (!isAlreadyFunction) {
        errors.push('Action "' + action.name + '" 代码格式错误（需要 function 定义）');
        continue;
      }
      // 检查花括号是否大致平衡
      var openBraces = (action.rawCode.match(/\{/g) || []).length;
      var closeBraces = (action.rawCode.match(/\}/g) || []).length;
      if (openBraces === 0 || openBraces !== closeBraces) {
        errors.push('Action "' + action.name + '" 代码花括号不匹配');
        continue;
      }
